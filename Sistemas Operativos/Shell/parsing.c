...
expand_environ_var(char *arg)
{
	...
	char *env_var = getenv(arg + 1);  // Next position after '$'.
	if (env_var == NULL) {
		arg[0] = '\0';
	} else {
		size_t len_env_var = strlen(env_var);
		if (len_env_var > strlen(arg)) {
			char *new_arg =
			        realloc(arg, 1 + sizeof(char) * len_env_var);
			if (new_arg == NULL) {
...
parse_exec(char *buf_cmd)
{
	...
	while (buf_cmd[idx] != END_STRING) {
		...
		if (tok[0] != '\0') {  // If env_var wasn't ok, skips it
			c->argv[argc++] = tok;
		}
	}
...
parse_line(char *buf)
{
	...
	if (strchr(right, '|') == NULL) {
		r = parse_cmd(right);  // If there's no | goes to parse_cmd.
	} else {
		r = parse_line(right);  // If strchr finds a | goes recursively to parse_line again.
	}

...
public class JwtService {
    ...
    public String createToken(JwtUserDetails claims) {
        return Jwts.builder()
                ...
                .claim(JWT_CLAIM_ROLE, claims.role())
    ...
    Optional<JwtUserDetails> extractVerifiedUserDetails(String token) {
        try {
            Claims claims = Jwts.parser()
                    ...
                    .getPayload();
            if (claims.containsKey(JWT_CLAIM_SUBJECT)
                    && claims.containsKey(JWT_CLAIM_ROLE)
                    && claims.get(JWT_CLAIM_ROLE) instanceof String role
            ) {
